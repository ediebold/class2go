...
def auth_view_wrapper(view):
    ...
    def inner(request, *args, **kw):
        ...
        if user.is_authenticated() and not is_member_of_course(course, user):
            ...
            return HttpResponseRedirect(reverse('courses.preview.views.preview', args=(request.common_page_data['course_prefix'], request.common_page_data['course_suffix'],)))

        if not user.is_authenticated():
            messages.add_message(request,messages.ERROR, 'You must be logged-in to view the content you chose.')
            
            return HttpResponseRedirect(reverse('accounts.views.default_login'))
